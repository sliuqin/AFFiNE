...
impl ShareableContent {
  ...
  pub fn tappable_application_with_process_id(
    ...
    // Find the TappableApplication with this process ID in the list of running applications
    match self.applications() {
      Ok(apps) => {
        ...
        // If we couldn't find a TappableApplication with this process ID, create a new one
        // with a default object_id of 0 (which won't be able to tap audio)
        match Application::new(process_id as i32) {
